...
#define Soft_I2C_DELAY 				Soft_I2C_Delay(100000)  /* 72M */
#define Soft_I2C_NOP					Soft_I2C_Delay(10)      /* 72M */   /* 约300k */
#define Soft_I2C_tAck         500                     /* 72M */
//#define Soft_I2C_DELAY 				Soft_I2C_Delay(10000)   /* 8M */
//#define Soft_I2C_NOP					Soft_I2C_Delay(1)       /* 8M */
//#define Soft_I2C_tAck         60                      /* 8M */
//#define Soft_I2C_DELAY 				Soft_I2C_Delay(70000)     /* 48M */
//#define Soft_I2C_NOP					Soft_I2C_Delay(7)         /* 48M */
//#define Soft_I2C_tAck         350                       /* 16M */

static void Soft_I2C_Delay(uint32_t dly);
static void Soft_I2C_Reset(void);
...
#define ST_Sensors_I2C_WriteRegister    Soft_I2C_Write
#define ST_Sensors_I2C_ReadRegister     Soft_I2C_Read
...
static void I2C_Mode_Configu(void)
{
  ...
  I2C_InitStructure.I2C_DutyCycle = I2C_DutyCycle_16_9;     /* SCL 时钟线的占空比 */
...
//I2C_Send7bitAddress(macI2Cx, (Address<<1), I2C_Direction_Transmitter);
	I2C_Send7bitAddress(macI2Cx, Address, I2C_Direction_Transmitter);
	I2CTimeout = I2CT_FLAG_TIMEOUT;
...
//I2C_Send7bitAddress(macI2Cx, (Address<<1), I2C_Direction_Receiver);
	I2C_Send7bitAddress(macI2Cx, Address, I2C_Direction_Receiver);
	I2CTimeout = I2CT_FLAG_TIMEOUT;
	/* Test on EV6 and clear it */
...
static void Soft_I2C_Reset(void)
{
	static uint8_t I2C_i;

	/* I2C_BUS reset */
	Soft_I2C_SDA_1; Soft_I2C_NOP;
	Soft_I2C_SCL_0; Soft_I2C_NOP;
	Soft_I2C_SDA_1; Soft_I2C_NOP;
	Soft_I2C_SCL_1; Soft_I2C_NOP;
	Soft_I2C_SDA_0; Soft_I2C_NOP;
	Soft_I2C_SCL_0; Soft_I2C_NOP;
	Soft_I2C_SDA_1; Soft_I2C_NOP;
	for(I2C_i=0; I2C_i<9; I2C_i++)
	{
		Soft_I2C_SCL_1; Soft_I2C_NOP;
		Soft_I2C_SCL_0; Soft_I2C_NOP;
	}
...
static void Soft_I2C_Configuration(void)
{
  ...
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_10MHz;              /* 设置I2C口最大允许输出速度 */
...
/**
  * @brief  软件I2C 发送8bit数据
  * @param  soft_i2c_data: 等待发送的字节
  * @retval none
  */
void Soft_I2C_SendByte(uint8_t soft_i2c_data)
{
 	static uint8_t i;
...
/**
  * @brief  软件I2C 读取一个8bit数据
  * @param  none
  * @retval soft_i2c_data: 读取到的数据
  */
uint8_t Soft_I2C_ReceiveByte(void)
{
	static uint8_t i,soft_i2c_data;
...
/**
  * @brief  检测I2C总线设备,CPU发送设备地址
  * @param  none
  * @retval 0 表示正确,返回1表示未探测到
  */
uint8_t Soft_I2C_CheckDevice(uint8_t _Address)
{
	...
	Soft_I2C_SendByte(_Address << 1 | I2C_Direction_Transmitter);
	ucAck = Soft_I2C_Wait_Ack();	/* 检测设备的ACK应答 */
...
{
	static uint8_t i, result=0;
	
	result  = Soft_I2C_START();
  if(result != 0) return result;
//result  = Soft_I2C_SendByte(soft_dev_addr | I2C_Direction_Transmitter);
	Soft_I2C_SendByte(soft_dev_addr << 1 | I2C_Direction_Transmitter);
	result  = Soft_I2C_Wait_Ack();
	if(result != 0) return result;

	Soft_I2C_SendByte(soft_reg_addr);
	result  = Soft_I2C_Wait_Ack();
	if(result != 0) return result;

	for (i=0;i<soft_i2c_len;i++)
	{
		Soft_I2C_SendByte(soft_i2c_data_buf[i]);
    result  = Soft_I2C_Wait_Ack();
		if (result != 0) return result;
	}
	Soft_I2C_STOP();
...
//result  = Soft_I2C_SendByte(soft_dev_addr | I2C_Direction_Transmitter);
	...
	Soft_I2C_START();
//result = Soft_I2C_SendByte(soft_dev_addr | I2C_Direction_Receiver);
	Soft_I2C_SendByte(soft_dev_addr << 1 | I2C_Direction_Receiver);
	result  = Soft_I2C_Wait_Ack();
	if(result != 0) return result;
...
//		else *soft_i2c_data_buf =Soft_I2C_ReceiveByte_WithACK();
    *soft_i2c_data_buf = Soft_I2C_ReceiveByte();
		if(soft_i2c_len == 1) Soft_I2C_SendNACK();
		else Soft_I2C_SendACK();
		soft_i2c_data_buf++;
		soft_i2c_len--;
	}
	Soft_I2C_STOP();
