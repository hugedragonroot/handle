...
#define Soft_I2C_DELAY 				Soft_I2C_Delay(1000000)  /* 72M */
#define Soft_I2C_NOP					system_driver_delay_us(1)      /* 72M */   /* 约300k */
...
uint8_t Soft_I2C_CheckDevice(uint8_t _Address)
{
	...
	Soft_I2C_SendByte(_Address << 1 | 0);
...
//result  = Soft_I2C_SendByte(soft_dev_addr | I2C_Direction_Transmitter);
	Soft_I2C_SendByte(soft_dev_addr << 1 | 0);
...
//result = Soft_I2C_SendByte(soft_dev_addr | I2C_Direction_Receiver);
	Soft_I2C_SendByte(soft_dev_addr << 1 | 1);
	result  = Soft_I2C_Wait_Ack();
	if(result != 0) return result;
