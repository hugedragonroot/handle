...
//__attribute((aligned(4))) char AllocArray[AllocArraySize]__attribute__((section(".ARM.__at_0x24064000")));//需要在MPU保护区域内，用来支持DMA传输；400kb之后的112kb内存
////static char AllocArray[AllocArraySize] __attribute((aligned(4)));
//#else
//__align(4) //保证内存池四字节对齐x
...
//__attribute__((section(".RAM_D1")))__attribute((aligned(4))) char AllocArray[AllocArraySize];// __attribute__((section(".ARM.__at_0x24020000")));//需要在MPU保护区域内，用来支持DMA传输；400kb之后的112kb内存
...
{
	rt_system_heap_init(AllocArray, &AllocArray[AllocArraySize-1]);
}
...
/*---------------------------------------------------------*/
void*xa_malloc_t(unsigned nbytes)
{
...
*/
void*xa_malloc_m(unsigned nbytes)
{
	void*p;
	//xa_log(LOG_DEBUG, "malloc:%d\r\n", nbytes);
	
	p = xa_malloc_t(nbytes);

	return p;
}


void xa_free_m(void*ap)
{
	if(ap==NULL)
		return;
...
void*xa_calloc(size_t n, size_t size)
{
	void *p;

	//xa_log(LOG_DEBUG, "xa_calloc\r\n");

	p = xa_malloc_t(n*size);
...
void *xa_realloc(void *ap, unsigned int newsize)
{
	ALLOC_HDR*bp;
	
	unsigned nunits;
	
	//xa_log(LOG_DEBUG, "xa_realloc: %d\r\n", newsize);

	if(ap == NULL)
	{
		bp = xa_malloc_t(newsize);
		return bp;	
	}
...
#ifdef ALLOC_TEST
void xa_malloc_test(void)
{
	char*p;

	p = (char*)xa_malloc(1024);

	/*打印指针，看看是不是4字节对齐*/
	xa_log(LOG_FUN, "pointer :%08x\r\n", p);

	memset(p, 0xf0, 1024);
	xa_log(LOG_FUN, "data:%02x\r\n", * (p+1023));
	xa_log(LOG_FUN, "data:%02x\r\n", * (p+1024));

	xa_free(p);
	xa_log(LOG_FUN, "alloc free ok\r\n");
